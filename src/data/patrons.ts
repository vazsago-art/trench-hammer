...
const ADMINISTRATUM_PREFECTUS: Patron = {
  ...
  abilities: [
    ...
    { name: 'Legal Action', description: 'At the start of the battle, after the deployment, select one enemy model. Redeploy that model anywhere on the battlefield (not into Melee combat, impassable terrain, or out of the battlefield).' },
    ...
    { name: 'Threat Response', description: 'Once during each Turn, when an enemy ends its movement within 8" of this model, this model may immediately make an attack with a ranged weapon (not a HEAVY weapon) against that enemy if otherwise able.' },
  ],
};
...
const CHAOS_UNDIVIDED: Patron = {
  ...
  abilities: [
    ...
    { name: 'Diabolist', description: "This model's psychic powers that cause Injuries (including attacks) have +1 DICE.", condition: 'Psyker Only' },
...
const CHAOS_WARLORD: Patron = {
  ...
  abilities: [
    { name: 'Flames of Spite', description: 'With a successful Risky Success Roll, this model can choose one enemy within 6" that it can see. That model gains 1 BLOOD MARKER and must roll on the Injury chart with the FIRE Keyword. This ability cannot be used during a Fireteam Activation.' },
...
const PATRON_KHORNE: Patron = {
  ...
  abilities: [
    ...
    { name: 'Brazen Hide', description: "The first -1 of this model's Armour gains IMPERVIOUS." },
...
const PATRON_NURGLE: Patron = {
  ...
  abilities: [
    ...
    { name: 'Heaving Mass', description: 'This model gains the LARGE Keyword if it did not already have it, and whenever it successfully Charges one or more enemies, each of those enemies gains a BLOOD MARKER.' },
...
const PATRON_TZEENTCH: Patron = {
  ...
  abilities: [
    { name: 'Aetherstride', description: 'This model ignores Difficult and Dangerous Terrain, and does not suffer Injury rolls due to falling. When this model makes a Diving Charge, it automatically passes the Diving Charge Success Roll.' },
    ...
    { name: 'Incorporeal Form', description: 'Injury rolls made against this model have an additional -1 DICE unless they have the FIRE, GAS, or PSYCHIC Keyword. STUN MARKERS have no effect on this model.' },
...
const HERETEK_LORD: Patron = {
  ...
  abilities: [
    { name: 'Architect Of Destruction', description: "This model's weapons with BLAST increase their BLAST radius by 1\"." },
    { name: 'Bastion', description: 'Enemies that Charged during this Turn have -1 INJURY DICE against this model, cannot force this model to move, and this model gains the BLOCK Keyword.' },
    { name: 'Daemonsmith', description: 'Friendly VEHICLE models within 6" of this model have IMPERVIOUS on their armour. This model counts as IMPERVIOUS for armour.' },
    ...
    { name: 'Siege Lord', description: 'If the target of any of this model\'s ranged attack would have an armour modifier of -3, you may set one die of the injury roll to a 6 before the roll is made. The rest of the dice for the injury roll are rolled as normal.' },
...
const PIRATE_LORD: Patron = {
  ...
  abilities: [
    { name: 'The Boot', description: 'Downed Enemy models in Melee Combat with this model cannot Stand Up by any means.' },
  ...
  adeptus_astartes: [
    {
      ...
      abilities: [
        ...
        // Codex / no subfaction
        { name: 'Adept Of The Codex', description: 'LIMIT: 1. While this model is on the Battlefield and is not Down, you cannot fail Morale tests.', condition: 'No Subfaction' },
        { name: 'Master Of Strategy', description: 'LIMIT: 1. After you activate this model, you decide the next 3 models to activate, following the normal limits (i.e. the second will usually be your own model again). If you choose a model in a Fireteam, the model\'s owner decides if it will activate as a Fireteam or individually.', condition: 'No Subfaction' },
        { name: 'Warden of The Imperium', description: 'When an enemy ends its movement within 6" of this model, this model can immediately make a single ranged attack against that enemy if it is otherwise able to do so.', condition: 'No Subfaction' },
        // Black Templars
        { name: 'Epitome Of Piety', description: 'This model can Deny the Witch as if it had PSYKER 2. If it is already a PSYKER, it has +1 DICE when Denying the Witch.', condition: 'Black Templars Only' },
        ...
        { name: 'Soulwarden', description: 'The first -1 of armour of this model and each friendly ASTARTES model within 3" of it have IMPERVIOUS.', condition: 'Blood Angels Only' },
        // Dark Angels
        ...
        // Deathwatch
        { name: 'Castellan Of The Black Vault', description: 'When you choose this skill, you can purchase any battlekit from your Campaign Shop, with its cost reduced by 3 Glory, but any battlekit purchased in this way can be equipped only by this model.', condition: 'Deathwatch Only' },
        ...
        { name: 'The Ties That Bind', description: 'LIMIT: 1. This model may form a FIRETEAM with any one non-ELITE model of the Warband. In addition, the first time either model fails a Risky Success Roll during a Fireteam Activation, the Activation does not end.', condition: 'Deathwatch Only' },
        // Grey Knights
        ...
        { name: 'Siege Master', description: "If the target of any of this model's ranged attack would have an armour modifier of -3, you may set one die of the injury roll to a 6 before the roll is made. The rest of the dice for the injury roll are rolled as normal.", condition: 'Imperial Fists Only' },
        // Iron Hands
        ...
        { name: 'Shadowmaster', description: 'While this model is in cover from such an attack, enemies cannot make attacks against this model at Long Range (unless the attack specifically ignores Long Range penalties).', condition: 'Raven Guard Only' },
        // Salamanders
        { name: 'Lord Of Fire', description: "This model's attacks with the FIRE Keyword have +1 INJURY DICE.", condition: 'Salamanders Only' },
        { name: 'Miraculous Constitution', description: 'When this model rolls on the Elite Trauma Chart, roll twice and choose either result.', condition: 'Salamanders Only' },
  ...
  adepta_sororitas: [
    {
      ...
      abilities: [
        ...
        { name: 'Executioner Of Heretics', description: 'Whenever this model takes an enemy model Out of Action with a Bloodbath result, you earn a Miracle, in addition to any you would normally earn.' },
        { name: 'Indomitable Belief', description: 'This model cannot be forced to move. In addition, if this model is Downed within 1" of an edge that would cause a falling Injury, it does not fall.' },
        ...
        { name: 'Pure Of Will', description: 'This model can Deny the Witch as if it had PSYKER 2. If it is already a PSYKER, it has +1 DICE when Denying the Witch.' },
  ...
  astra_militarum: [
    {
      ...
      abilities: [
        ...
        { name: 'Faithful Servant of the Throne', description: 'This model can Deny the Witch as if it had PSYKER 2. If it is already a PSYKER, it has +1 DICE when Denying the Witch.' },
        ...
        { name: 'Master Tactician', description: 'LIMIT: 1. After you activate this model, you decide the next 3 models to activate, following the normal limits (i.e. the second will usually be your own model again). If you choose a model in a Fireteam, the model\'s owner decides if it will activate as a Fireteam or individually.' },
      ],
    },
  ...
  rogue_trader: [
    {
      ...
      abilities: [
        { name: 'Acquisitions', description: 'While this model is in your Warband, your credit limit for missions is increased by 25.', condition: 'LIMIT: 1' },
  ...
  necromunda_gang: [
    {
      ...
      abilities: [
        { name: 'Crushing Blow', description: 'When this model Downs an enemy with a melee attack (even if it is already Down), that enemy suffers an additional BLOOD MARKER.' },
        ...
        { name: 'Mighty Leap', description: 'This model automatically succeeds when it takes the Jump Success Roll. In addition, this model has +1 DICE to Diving Charge Success Rolls.' },
    ...
    },
    {
      ...
      abilities: [
        ...
        { name: 'Down and Dirty', description: 'This model ignores the penalties to Success Roll rolls from being Down. Additionally, enemies do not gain bonuses against this model when it is Down.' },
        ...
        { name: 'Lie Low', description: 'While this model is in cover against such an attack, enemies cannot make ranged attacks at Long Range against it (unless the attack specifically ignores Long Range penalties).' },
    ...
    },
    {
      ...
      abilities: [
        ...
        { name: 'Inspirational', description: 'This model and its allies within 8" have the NEGATE FEAR Keyword.' },
  ...
  heretic_astartes: [
    {
      ...
      abilities: [
        { name: 'Clandestine', description: 'Ranged attacks made against this model automatically miss on a result of 7 only.' },
        { name: 'Covert Control', description: 'At the start of the battle, after the deployment, select one enemy model. Redeploy that model anywhere on the battlefield (not into Melee combat, impassable terrain, or out of the battlefield).' },
        { name: 'Cult Leader', description: 'Friendly Cultist models within 3" of this model have +1 DICE to Hit with all attacks.' },
        { name: 'Headhunter', description: "This model's attacks cannot have their target changed, Injury rolls caused by them cannot have -DICE applied to them, and this model ignores range reductions when targeting enemies." },
    ...
    },
    {
      ...
      abilities: [
        { name: 'Dirty Fighter', description: 'Once per Turn, whenever an enemy model misses this model with a melee attack, this model can make a single melee attack against that enemy.' },
        ...
        { name: "Night Haunter's Curse", description: 'As an Action with a Risky Success Roll, this model can choose one enemy model it can see within 6". On a success, that enemy must roll on the Injury chart, ignoring armour. This injury is NONLETHAL.' },
        { name: 'One Piece At A Time', description: 'Whenever this model hits with a melee attack, the target suffers 1 BLOOD MARKER.' },
        { name: 'One With The Shadows', description: 'While this model is in cover from such an attack, enemies cannot make attacks against this model at Long Range (unless the attack specifically ignores Long Range penalties).' },
      ],
    },
    CHAOS_UNDIVIDED,
    CHAOS_WARLORD,
  ...
  aeldari: [
    {
      ...
      abilities: [
        ...
        { name: 'Enduring Resolve', description: 'This model can Deny the Witch as if it had PSYKER 2. If it is already a PSYKER, it has +1 DICE when Denying the Witch.' },
    ...
    },
    {
      ...
      abilities: [
        ...
        { name: 'Shield of Grace', description: 'Melee attacks against this model have -1 DICE to Hit.', condition: 'Dire Avenger Only' },
        // Fire Dragon
        ...
        { name: 'Shadow Hunter', description: 'While this model is in cover, enemies have an additional -1 DICE to Hit it with ranged attacks. While this model is not in cover, it gains the STEALTH Keyword.', condition: 'Striking Scorpion Only' },
  ...
  drukhari: [
    {
      ...
      abilities: [
        ...
        { name: 'Labyrinthine Cunning', description: 'LIMIT: 1. After you activate this model, you decide the next 3 models to activate, following the normal limits (i.e. the second will usually be your own model again). If you choose a model in a Fireteam, the model\'s owner decides if it will activate as a Fireteam or individually.' },
  ...
  genestealer_cults: [
    {
      ...
      abilities: [
        ...
        { name: 'Focus of Adoration', description: 'When this model is hit by a non-BLAST attack, it can choose one non-Elite ally within 1" of it. That model becomes the target of the hit instead, even if it would normally be out of range.' },
        ...
        { name: 'Shadow Stalker', description: 'This model gains the STEALTH Keyword and INFILTRATOR Keyword. While in cover, all attacks against it have an additional -1 DICE to Hit.' },
      ],
    },
  ],
  ...
  necrons: [
    {
      ...
      abilities: [
        { name: 'Enduring Will', description: 'All Down results on the Injury table count as Minor Hits against this model.' },
        { name: 'Eternal Madness', description: 'Injury rolls caused by the melee attacks of this model cannot have -DICE applied to them. This model also has +1 DICE to Hit rolls in melee.' },
        ...
        { name: 'Immortal Pride', description: 'Injuries caused by any attack (including those that ignore armour or have ARMOUR PIERCING 2) have -1 DICE against this model.' },
    ...
    },
    {
      id: 'obliteration',
      name: 'Nihilarch',
      description: "The drive to eradicate all organic life utterly — the Nihilarch creed of the Destroyer Cult. Used by Destroyer Cult warbands.",
      abilities: [
    ...
    },
    {
      ...
      abilities: [
        ...
        { name: 'Shadow of Drazak', description: 'All attacks made against this model have -1 DICE to Hit.' },
  ...
  orks: [
    {
      ...
      abilities: [
        ...
        { name: 'Ded Choppy', description: 'LIMIT: 1. While this model is in your Warband, the cost of any melee weapon worth 10 credits or more is reduced by 5 credits. This effect does not stack. Note that weapons bought with this lower cost will make it easier for you to fit under the Threshold value.' },
      ],
    },
    {
      id: 'mork',
      ...
      abilities: [
        { name: 'Dakkalad', description: 'LIMIT: 1. While this model is in your Warband, the cost of any ranged weapon worth 10 credits or more is reduced by 5 credits. This effect does not stack. Note that weapons bought with this lower cost will make it easier for you to fit under the Threshold value.' },
  ...
  tau_empire: [
    ...
    },
    {
      ...
      abilities: [
        ...
        { name: 'Tactical Acumen', description: 'While this model is in your Warband, you can increase the number of models that are normally limited (such as Crisis Battlesuits) by 1. This does not include ELITE Models. Once purchased, this extra model is part of the warband even if this skill is lost.', condition: 'LIMIT: 1' },
      ],
    },
    {
      id: 'shaper_chief',
      ...
      abilities: [
        ...
        { name: "Predator's Pounce", description: 'While Charging, this model ignores Difficult and Dangerous Terrain, it automatically passes any Climb or Jump Success Roll, and it automatically passes its Diving Charge Success Roll.' },
        { name: 'Prey Slayer', description: 'This model has +1 DICE to Hit and +1 INJURY DICE against enemies that are Down.' },
        { name: 'Technological Trophy', description: "You can purchase any one weapon that does not cost Glory for this model from the following Warbands' Armoury/Battlekit List: any Imperial or Outlaw Warband, Chaos Cult, Heretic Astartes, Aeldari, Drukhari, Genestealer Cults, Harlequins, or Leagues of Votann. Only this model can carry that weapon, and its credit cost is reduced by 5 credits, but it can be repurchased if lost." },
      ],
    },
  ],
  ...
  tyranids: [
    {
      ...
      abilities: [
        ...
        { name: 'Heightened Reflexes', description: 'Once during each Turn, when an enemy model ends its movement within 6" of this model, this model can make a single ranged attack against that enemy.' },
        { name: 'Synaptic Lynchpin', description: 'The model gains the SYNAPSE Keyword. If it already has SYNAPSE, instead allies are Within Synapse Range while they are within an additional 2" of this model.' },
  ...
  leagues_of_votann: [
    {
      ...
      abilities: [
        ...
        { name: 'Bequest of the Votann', description: 'When this model is selected, choose one friendly non-Elite model. That model gains +1 DICE to all Hit rolls for the rest of the battle.' },
        { name: 'Grim Demeanor', description: 'This model and its VOTANN allies within 8" of it ignore the FEAR Keyword of enemies.' },
        ...
        { name: 'Warrior Lord', description: 'Injury rolls caused by the melee attacks of this model cannot have -DICE applied to them. This model also has +1 DICE to Hit rolls in melee.' },
      ],
    },
    {
      id: 'guildmaster',
      ...
      abilities: [
        ...
        { name: 'Experienced Eye', description: 'LIMIT: 1. The cost of any suit of armour or equipment worth 15 credits or more is reduced by 5 credits if the model with this skill is part of the Warband. This does not stack.' },
        { name: 'Guild Affiliate', description: 'At the end of each battle, you earn an additional 2D6x10 credits.' },
  ...
  slanni: [
    {
      ...
      abilities: [
        ...
        { name: 'Higher State of Consciousness', description: 'LIMIT: 1. After you activate this model, you decide the next 3 models to activate, following the normal limits (i.e. the second will usually be your own model again). If you choose a model in a Fireteam, the model\'s owner decides if it will activate as a Fireteam or individually.' },
  ...
  harlequins: [
    {
      ...
      abilities: [
        // Universal
        { name: 'A Foot in the Future', description: 'When this model declares a Charge, before rolling for charge distance, place this model anywhere within 1" of the target. If within Melee range, it can make a single melee attack as if in close combat. If that attack causes the enemy to suffer any BLOOD MARKERS, the Charge succeeds automatically.' },
        ...
        // Masque-specific
        { name: 'Player of the Light', description: 'This model always charges the maximum distance possible rather than rolling. In addition, this model may reroll its Diving Charge Success Roll.', condition: 'Light Only' },
        { name: 'Player of the Dark', description: 'The first time during each of its Activations that this model hits with a melee attack, the target suffers 1 additional BLOOD MARKER.', condition: 'Dark Only' },
...
// ── Warband-wide campaign effects ───────────────────────────────────────────

export type PatronWarbandEffectCategory = 'cost' | 'limit' | 'income' | 'battle';

export interface PatronWarbandEffect {
  category: PatronWarbandEffectCategory;
  summary: string;
}

/**
 * Patron skills that provide a passive warband-wide benefit during campaign phases.
 * Keyed by the exact ability name as it appears in PatronAbility.name.
 * Used by CampaignManager to surface "Active Warband Effects" when a unit holds these skills.
 */
export const PATRON_WARBAND_EFFECTS: Record<string, PatronWarbandEffect> = {
  // Cost reductions (Quartermaster Step)
  'Supply Shipment: Armour and Equipment': { category: 'cost',   summary: 'Armour & equipment costing ≥15cr is reduced by 5cr (LIMIT: 1 model).' },
  'Supply Shipment: Melee Weapons':        { category: 'cost',   summary: 'Melee weapons costing ≥10cr are reduced by 5cr (LIMIT: 1 model).' },
  'Supply Shipment: Ranged Weapons':       { category: 'cost',   summary: 'Ranged weapons costing ≥20cr are reduced by 5cr (LIMIT: 1 model).' },
  'Experienced Eye':                       { category: 'cost',   summary: 'Armour & equipment costing ≥15cr is reduced by 5cr (LIMIT: 1 model).' },
  'Ded Choppy':                            { category: 'cost',   summary: 'Melee weapons costing ≥10cr are reduced by 5cr.' },
  'Dakkalad':                              { category: 'cost',   summary: 'Ranged weapons costing ≥10cr are reduced by 5cr.' },
  'Firearm Procurement':                   { category: 'cost',   summary: 'Ranged weapons costing ≥20cr are reduced by 5cr (LIMIT: 1 model).' },
  // Limit increases (Reinforcements Step)
  'Organizational Talent':                 { category: 'limit',  summary: 'LIMIT of one chosen battlekit item increased by 1 (record your choice).' },
  'Secret Forces':                         { category: 'limit',  summary: 'LIMIT of one Troop type increased by 1 (not LIMIT:1 or LARGE models).' },
  'Port Contacts':                         { category: 'limit',  summary: 'LIMIT of any one weapon, armour, or equipment increased by 1.' },
  'Tactical Acumen':                       { category: 'limit',  summary: 'Limit of one non-ELITE model type increased by 1.' },
  'Aspect Armoury':                        { category: 'limit',  summary: 'LIMIT of one Aspect-restricted battlekit item increased by 1 (record your choice).' },
  'Aspect Mastery':                        { category: 'limit',  summary: 'Recruit one extra Aspect Warrior of the chosen Aspect (requires ≥2 already in warband).' },
  // Glory & credit income (end of battle)
  'Guild Affiliate':                       { category: 'income', summary: 'Earn +2D6×10 credits at the end of each battle.' },
  'Trusted War Leader':                    { category: 'income', summary: 'Earn +1 Glory at the end of each battle. (Leader Only)' },
  'Inquisitorial Honor':                   { category: 'income', summary: 'Earn +1 Glory at the end of each battle this model participated in. (non-Inquisition Only)' },
  'Acquisitions':                          { category: 'income', summary: 'Credit limit for missions is increased by 25.' },
  // Battle setup effects
  'Master of the Void':                    { category: 'battle', summary: 'At battle start, up to 2 non-Elite ROGUE TRADER models gain DEEP STRIKE until end of battle.' },
  'Master of Spies':                       { category: 'battle', summary: 'Roll twice for your Shadow Operation at the start of each battle; choose one result. (Inquisition Only)' },
  'Legal Action':                          { category: 'battle', summary: 'After deployment, redeploy one enemy model anywhere on the battlefield (not into Melee, impassable terrain, or off the board).' },
};

export const PATRON_WARBAND_EFFECT_CATEGORY_LABELS: Record<PatronWarbandEffectCategory, string> = {
  cost:   '💰 Cost Reductions',
  limit:  '📋 Limit Increases',
  income: '⭐ Glory & Credit Income',
  battle: '⚔ Battle Setup',
};
